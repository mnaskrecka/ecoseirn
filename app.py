...
def model(t, y, params):
    ...
    dI_dt = (σ1 * M / (m + M) * (I + R) / N * E
             - γr * I + γi * R)
    dR_dt = (-γi * R + γr * I)
    dNE_dt = (βN * NE / N * S - α * M / (m + M) * (E + I + R) / N * NE)
    dM_dt = (r * (NE + S) / N - r0 * (M - Mo))
    return [dS_dt, dE_dt, dI_dt, dR_dt, dNE_dt, dM_dt]

st.title("Symulacja modelu dynamicznego – Green Consumer")
...
axs[0].set_title('S, E, I, R, NE w czasie')
axs[0].set_ylabel('Odsetek w populacji (%)')
axs[0].set_xticks(t)
...
axs[1].set_title('Intensywność przekazu medialnego M(t)')
axs[1].set_ylabel('Intensywność mediów')
axs[1].set_xticks(t)
